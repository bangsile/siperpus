<div class="mt-10 sm:mx-auto sm:w-full sm:max-w-sm">
    <form class="space-y-6" wire:submit="login" method="POST">
        <div>
            ...
            <div class="mt-2">
                <input wire:model="username" type="text" name="username" id="username" autocomplete="username"
        ...
        <div>
            ...
            <div class="mt-2">
                <input wire:model="password" type="password" name="password" id="password" autocomplete="current-password"
                    class="block w-full rounded-md bg-white px-3 py-1.5 text-base text-gray-900 outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline-blue-600 sm:text-sm/6" />
                @error('password')
                    <label class="block text-sm/6 font-medium text-red-600">{{$message}}</label>
                @enderror
        ...
        @if (session("error"))
            <label class="block text-sm font-medium text-red-600 text-center pb">Login Gagal. Silahkan coba lagi.</label>
        @endif
